...
#### (2)
(1)で作成したNumPyの配列の各要素を2乗してください。

#### (3)
`numpy.sum()`関数を使って、(3)で作成したNumPyの配列の要素の合計を計算してください。
...
課題4のプログラムでは、`x = np.zeros(num_item)`で初期解を設定しています。なぜこのような初期解が設定されているのか、説明してください。他の初期解を設定するとどのような問題が発生する可能性があるか、考察してください。

### 課題6

課題4のプログラムでは、`knapsack_weight(x_new) <= W and knapsack_value(x_new) > knapsack_value(x)`という条件で解を更新しています。この条件について、なぜこのような条件が設定されているのか、説明してください。他の条件を設定するとどのような問題が発生する可能性があるか、考察してください。
